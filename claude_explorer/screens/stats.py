...
class StatsScreen(Container):
    ...
    def load_stats(self) -> None:
        ...
        max_msgs = max(s.message_count for s in stats) if stats else 1
        max_tools = max(s.tool_call_count for s in stats) if stats else 1

        chart.write("[bold #cba6f7]Messages per Day[/]")
        chart.write("")

        # Show last 20 days for a nice chart
        recent = stats[-20:]
        for day in recent:
            bar = make_bar(day.message_count, max_msgs, 40)
            chart.write(
                f"  {day.date}  [#89b4fa]{bar}[/] {day.message_count}"
            )
        ...
        for day in recent:
            ...
            chart.write(
                f"  {day.date}  [#a6e3a1]{bar}[/] {day.tool_call_count}"
            )
        ...
        chart.write("")
        chart.write(f"[bold #cba6f7]Summary:[/]")
